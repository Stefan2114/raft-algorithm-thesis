...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        stepbox/.style={
            ...
            text width=6.6cm, minimum height=1.1cm, 
            align=center, thick, fill=white, font=\small
        },
        arrow/.style={-{Stealth}, thick, shorten >=2pt, shorten <=2pt}
    ]
        % Vertically stacked steps
        \node[stepbox, fill=blue!10] (Step1) at (0, 0) 
            {\textbf{1. Submission} \\ Client $\rightarrow$ gRPC Handler};
            
        \node[stepbox, fill=green!10, below=of Step1] (Step2) 
            {\textbf{2. Buffering} \\ gRPC $\rightarrow$ RSM Layer (\texttt{Submit})};
            
        \node[stepbox, fill=orange!10, below=of Step2] (Step3) 
            {\textbf{3. Log Replication} \\ RSM $\rightarrow$ Raft Core (\texttt{rf.Start()})};
            
        \node[stepbox, fill=purple!10, below=of Step3] (Step4) 
            {\textbf{4. Commitment} \\ Quorum achieved $\rightarrow$ \texttt{applyCh}};
            
        \node[stepbox, fill=red!10, below=of Step4] (Step5) 
            {\textbf{5. State Machine Execution} \\ \texttt{applyCh} $\rightarrow$ Store (\texttt{DoOp})};

        \node[stepbox, fill=teal!10, below=of Step5] (Step6) 
            {\textbf{6. Client Response} \\ RSM $\rightarrow$ gRPC Handler};
        ...
        \draw[arrow] (Step6.west) .. controls +(left:2.5) and +(left:2.5) .. node[left, font=\scriptsize, align=right] {Return Response} (Step1.west);
    \end{tikzpicture}
...
The system utilizes the Command pattern to represent client operations as discrete objects that can be replicated and executed.
In the RSM layer, every client request is encapsulated into an \texttt{Op} structure.

The use of the Command pattern allows the Raft log to store a sequence of opaque \texttt{Op} objects. When the \texttt{applier} routine retrieves these entries from the log, it does not need to distinguish between a \texttt{Get} or a \texttt{Put} operation; it simply passes the command to the state machine's \texttt{DoOp} method for execution.
...
The mechanism utilizes the \texttt{applyCh} as the notification medium. This decoupling is vital for system liveness; the Raft core can continue to process heartbeats and manage elections even if the RSM is temporarily blocked by a slow state machine operation, as the notification process is asynchronous.
...
The \texttt{RaftService} translates incoming gRPC messages, such as \texttt{AppendEntriesArgs}, into the internal structures required by the \texttt{Raft} module. This ensures that the core consensus logic is not "polluted" with networking-specific code or third-party protobuf types, making the system easier to maintain and test in isolation.

\subsection{The Strategy Pattern: Pluggable Persistence}
To support different storage backends, the implementation employs the Strategy pattern for its persistence layer.
The Raft core does not interact with the file system directly; instead, it relies on a \texttt{Persister} interface.

This design highlights the flexibility of the architecture. In production, a \texttt{FilePersister} is used to ensure durability across restarts. However, during development and continuous integration, a memory-based mock strategy can be injected into the Raft core, allowing for significantly faster test execution and the ability to simulate disk failures or corruption scenarios without physical hardware constraints.
...
To keep storage sizes small and recovery times fast, the node stores consistent checkpoints on disk. The system creates periodic snapshots of the state machine, allowing the active node to discard older log segments. The persister saves both the Raft terms and the latest snapshots to non-volatile storage, which allows the node to recover quickly after a crash without corrupting data.
...
\begin{enumerate}
    ...
    \item \textbf{Truncation}: The snapshot is passed to the Raft core (\texttt{rf.Snapshot}), which then discards all log entries up to the snapshot index and updates its \texttt{lastIncludedIndex}.
\end{enumerate}
...
The system's default strategy is the \texttt{FilePersister} \cite{disk_persister_go}. This implementation ensures durability by using an \texttt{atomicWrite} approach:
\begin{itemize}
