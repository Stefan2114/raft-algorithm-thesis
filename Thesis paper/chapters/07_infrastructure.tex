...
\begin{figure}[h]
    \centering
    % 
    % TODO: Add a sequence diagram or block diagram showing the Java Clerk 
    % calling the Go gRPC Server, handling a WrongLeader error, and retrying.
    \fbox{\begin{minipage}{0.8\textwidth}
        \centering
        \vspace{2cm}
        [Diagram: Java-Go Cross-Language Interaction] \\
        (Java Clerk $\xrightarrow{gRPC}$ Go Node $\rightarrow$ Hint Response $\rightarrow$ Java Retry)
        \vspace{2cm}
    \end{minipage}}
    \caption{The cross-platform communication flow. The gRPC layer abstracts the language differences, allowing the Java client to participate in the Raft-based protocol seamlessly.}
    \label{fig:cross_lang_interop}
\end{figure}

The success of this integration, as illustrated in Figure \ref{fig:cross_lang_interop}, proves that the Raft implementation is not tied to a specific ecosystem. By relying on industry-standard protocols, the system can support a diverse range of clients, from low-latency Go microservices to heavy-duty Java enterprise applications.
...
\begin{figure}[h]
    \centering
    % TODO: Add a diagram of the Multi-Stage Docker Build.
    % Show [Go SDK + Source] -> [Compiler] -> [Static Binary] -> [Minimal Alpine Image].
    \fbox{\begin{minipage}{0.8\textwidth}
        \centering
        \vspace{2cm}
        [Diagram: Multi-Stage Docker Build Process] \\
        (From 800MB+ Build Environment to <20MB Final Image)
        \vspace{2cm}
    \end{minipage}}
    \caption{The multi-stage build pipeline for the KV server, ensuring an optimized and secure production image.}
