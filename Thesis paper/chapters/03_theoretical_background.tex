...
In a distributed environment, physical clocks are unreliable and cannot be used to coordinate event order across different machines. Raft addresses this by dividing time into \textbf{terms} of arbitrary length, as illustrated in Figure \ref{fig:raft_terms}.

\begin{figure}[h]
    \centering
    % TODO: Add a diagram showing Raft terms. 
    % The diagram should show a timeline split into Term 1, Term 2, etc. 
    % Some terms have a successful election and a leader period; others (split votes) have only an election.
    \fbox{\begin{minipage}{0.8\textwidth}
        \centering
        \vspace{2cm}
        [Diagram: Raft Time Scale - Sequence of Terms] \\
        (Term 1: Election + Normal Operation | Term 2: Split Vote | Term 3: Election + Normal Operation)
        \vspace{2cm}
    \end{minipage}}
    \caption{The division of time into terms in Raft. Each term acts as a logical clock to detect obsolete information.}
    \label{fig:raft_terms}
\end{figure}

As shown in Figure \ref{fig:raft_terms}, each term begins with an election. If a candidate wins the election, it serves as the leader for the remainder of the term. In some cases, an election might result in a "split vote," where no candidate achieves a majority. In such instances, the term ends immediately, and a new term begins with another election. 
...
At any given time, a Raft node exists in one of three states: \textbf{Follower}, \textbf{Candidate}, or \textbf{Leader}. The transitions between these states are governed by election timeouts and voting results, depicted in Figure \ref{fig:raft_state_machine}.
...
\begin{figure}[h]
    ...
    % Candidate/Leader -> (discovers higher term) -> Follower
    \fbox{\begin{minipage}{0.8\textwidth}
        \centering
        \vspace{2cm}
        [Diagram: Raft State Transition Machine] \\
        (Follower $\leftrightarrow$ Candidate $\rightarrow$ Leader)
        \vspace{2cm}
    \end{minipage}}
    \caption{State machine transitions for a Raft node. Nodes transition from Follower to Candidate upon an election timeout.}
    \label{fig:raft_state_machine}
\end{figure}

The process of election, as shown in the transitions of Figure \ref{fig:raft_state_machine}, follows a specific sequence:
\begin{enumerate}
...
The structure of the Raft log is fundamental to the algorithm's safety. As shown in Figure \ref{fig:raft_log_structure}, each log entry stores a state machine command along with the term number when the entry was received by the leader.

\begin{figure}[h]
    \centering
    % TODO: Add a diagram of the Raft Log Structure.
    % Should show a sequence of boxes with Index, Term, and Command (e.g., x<-3).
    \fbox{\begin{minipage}{0.8\textwidth}
        \centering
        \vspace{2cm}
        [Diagram: Raft Log Structure] \\
        (Log showing Index 1..N, Terms, and Commands like SET x=1, DEL y)
        \vspace{2cm}
    \end{minipage}}
    \caption{The structure of a Raft log. Each entry is identified by an index and a term, which are used to identify inconsistencies between logs.}
    \label{fig:raft_log_structure}
\end{figure}

The log structure depicted in Figure \ref{fig:raft_log_structure} allows the algorithm to maintain the \textit{Log Matching Property}. This property states that if two entries in different logs have the same index and term, then they store the same command and all preceding entries in the logs are identical. This property is enforced through a consistency check performed during every \texttt{AppendEntries} RPC.
...
Raft uses the voting process to prevent a candidate from winning an election unless its log contains all committed entries. Since a candidate must contact a majority of the cluster to win, and every committed entry resides on at least a majority of nodes, there is guaranteed to be at least one node that possesses every committed entry.

As illustrated in Figure \ref{fig:election_restriction}, the voting mechanism includes a strict comparison of log "up-to-dateness."

\begin{figure}[h]
    \centering
    % TODO: Add a diagram showing the Election Restriction check.
    % Voter (lastIndex, lastTerm) vs Candidate (lastIndex, lastTerm).
    % Show the logic: (CandidateTerm > VoterTerm) OR (CandidateTerm == VoterTerm AND CandidateIndex >= VoterIndex)
    \fbox{\begin{minipage}{0.8\textwidth}
        \centering
        \vspace{2cm}
        [Diagram: Raft Election Restriction Logic] \\
        (Candidate Log vs. Voter Log comparison during RequestVote)
        \vspace{2cm}
    \end{minipage}}
    \caption{The Election Restriction mechanism. Voters deny requests if the candidate's log is less complete than their own.}
    \label{fig:election_restriction}
\end{figure}

The comparison logic shown in Figure \ref{fig:election_restriction} follows two simple rules:
\begin{enumerate}
...
A subtle but vital safety rule in Raft is that a leader cannot conclude that an entry from a \textit{previous} term is committed just because it is stored on a majority of servers. Only entries from the leader's \textit{current} term can be committed by counting replicas.

This safety requirement is depicted in Figure \ref{fig:safety_edge_case}, which demonstrates a potential hazard where an old entry could be overwritten if this rule were not in place.

\begin{figure}[h]
    \centering
    % TODO: Recreate Figure 8 from the Raft paper.
    % Show a scenario where an entry from Term 2 is replicated on a majority in Term 4, 
    % but then overwritten by a leader from Term 5.
    \fbox{\begin{minipage}{0.8\textwidth}
        \centering
        \vspace{2cm}
        [Diagram: Safety Edge Case - Committing Previous Terms] \\
        (Timeline showing Term 2 entry being overwritten by Term 5 leader)
        \vspace{2cm}
    \end{minipage}}
    \caption{A scenario illustrating why leaders only commit entries from their current term by counting replicas. This prevents the "overwriting" of entries that appeared to be committed but were not actually safe.}
    \label{fig:safety_edge_case}
\end{figure}

As shown in Figure \ref{fig:safety_edge_case}, once a leader has committed at least one entry from its current term, the Log Matching Property ensures that all prior entries in the log are also committed. This "indirect commitment" is the only safe way to finalize the status of entries inherited from previous leaders.
...
Raft was explicitly designed to be more "understandable" than Paxos while maintaining the same performance profile. The core philosophy of the shift toward Raft is the decomposition of the consensus problem into independent subproblems (Election, Replication, and Safety) and the enforcement of a stronger degree of coherency.

As shown in Figure \ref{fig:paxos_vs_raft_log}, the fundamental difference in log management lies in the "flow" of data.

\begin{figure}[h]
    \centering
    % TODO: Add a diagram comparing Paxos and Raft log flow.
    % Paxos side: Any node proposing to any slot, potential holes in log.
    % Raft side: Leader-only append, strict sequential order (prefix matching).
    \fbox{\begin{minipage}{0.8\textwidth}
        \centering
        \vspace{2cm}
        [Diagram: Comparison of Log Replication Models] \\
        (Paxos: Symmetric Proposers $\leftrightarrow$ Raft: Strong Leader Flow)
        \vspace{2cm}
    \end{minipage}}
    \caption{A comparison of log replication logic. Raft enforces a strict flow from leader to followers, whereas Paxos allows more symmetric, but complex, interaction.}
    \label{fig:paxos_vs_raft_log}
\end{figure}

The mechanism illustrated in Figure \ref{fig:paxos_vs_raft_log} highlights Raft's "Strong Leader" model. Unlike Paxos, where log entries can be committed out of order or "holes" can appear in the log, Raft ensures that logs always remain a continuous prefix of the leader's log. This simplification reduces the number of potential states the system can be in, making it easier to verify and implement correctly.
